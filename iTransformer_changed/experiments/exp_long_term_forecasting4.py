...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        ...
        return optim.AdamW(optimizer_grouped_parameters, lr=self.args.learning_rate)
    
    # cambio para mejorar el ajuste del lr reemplazando adjust_learning_rate con schedulers modernos
    def _select_scheduler(self, optimizer):
        """Reemplaza adjust_learning_rate con schedulers modernos"""
        
        if self.args.lradj == 'type1':
            # Equivalente a type1: decay exponencial
            scheduler = optim.lr_scheduler.ExponentialLR(
                optimizer, 
                gamma=0.5  # Reduce a la mitad cada época
            )
            
        elif self.args.lradj == 'type2':
            # Equivalente a type2: milestones específicos
            ...
            # Nuevo: Adaptive basado en validación (MEJOR)
            ...
            # Nuevo: Warmup + Cosine (ÓPTIMO)
            scheduler = self._get_warmup_cosine_scheduler(optimizer)
            
        else:
            # 'none' o cualquier otro valor
            scheduler = None
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    ...
                    loss.backward()
                    grad_norm= torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=3.0)
                    model_optim.step()
                # guardar grad_norm para diagnóstico
            ...
            if scheduler is not None:
                if isinstance(scheduler, optim.lr_scheduler.ReduceLROnPlateau):
                    scheduler.step(vali_loss)  # Necesita métrica
                else:
                    scheduler.step()  # Solo necesita época
            ...
            #adjust_learning_rate(model_optim, epoch + 1, self.args) -- reeemplazoado por schedulers modernos
